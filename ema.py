...
def main():
    ...
    with st.sidebar:
        ...
        config_status = []
        if dhan_token: 
            config_status.append("✅ DhanHQ Token")
        else: 
            config_status.append("❌ DhanHQ Token")
        
        if telegram_token: 
            config_status.append("✅ Telegram Bot")
        else: 
            config_status.append("❌ Telegram Bot")
        
        if telegram_chat_id: 
            config_status.append("✅ Chat ID")
        else: 
            config_status.append("❌ Chat ID")
        
        if github_csv_url: 
            config_status.append("✅ Stocks CSV")
        else: 
            config_status.append("❌ Stocks CSV")
        ...
        if st.session_state.last_scan:
            time_since_last = (datetime.now() - st.session_state.last_scan).total_seconds() / 60
            next_scan_in = max(0, scan_interval - time_since_last)
            st.markdown(f"**Next Auto-Scan:** {next_scan_in:.0f} min")
        ...
        app_url = "YOUR_STREAMLIT_APP_URL"  # Replace with actual URL
    ...
    if st.session_state.last_alerts:
        ...
        for key, timestamp in list(st.session_state.last_alerts.items())[-10:]:  # Last 10 alerts
            try:
                ...
                logger.warning(f"Error processing alert key {key}: {e}")
                continue
